...
- `reviewability` is read fresh on every `fetch` and every `watch` poll, so a `timed_out: true` result carries it too — check `state.reviewability.blockers` before issuing another `watch` call rather than waiting out another window for reviews that cannot arrive. `mergeable: "UNKNOWN"` is never reported as a blocker: GitHub computes mergeability in a background job and returns `UNKNOWN` while it runs, which is exactly the moment just after a push. It resolves on a later check.
