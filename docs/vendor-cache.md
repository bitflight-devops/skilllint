...
```bash
LATEST_PATH="$(uv run skilllint docs latest "$(uv run python -c 'from skilllint.vendor_cache import derive_page_name; print(derive_page_name("https://example.com/guide.md"))')")"
