...
1. Run `uv run python -m skilllint.plugin_validator check ~/repos/claude-plugins-official --no-color` and capture full output + exit code. Do the same for `~/repos/skills` and `~/repos/claude-code-plugins`.
...
- `uv run python -m skilllint.plugin_validator check ~/repos/claude-plugins-official --no-color; echo "exit: $?"` — exit code matches expectations
...
- Official repos at: `~/repos/claude-plugins-official`, `~/repos/skills`, `~/repos/claude-code-plugins`
