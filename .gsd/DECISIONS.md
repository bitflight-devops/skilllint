...
| D010 | M002 | scope | Demo-generation rollout order | Start with task-level summary videos before slice- or milestone-level rollups | Task videos are the smallest truthful replay unit and can compose upward later | No |
| D011 | M002 | arch | Evidence completeness strategy | Prove video generation from partial but real assets before capture completeness is finished | The user wants the pipeline working early without waiting for a perfect trace layer | Yes — revisit if missing evidence makes outputs unusable |
| D012 | M002 | constraint | Source-of-truth media policy | Existing dashboard capture and literal tmux footage are primary evidence; Remotion augments with transitions, overlays, pacing, and explanation | The user wants videos grounded in real UI/terminal evidence rather than reconstructed playback | No |
