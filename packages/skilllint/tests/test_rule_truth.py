...
from skilllint.plugin_validator import FM003, FM004, FM005, FM007, ErrorCode, FrontmatterValidator, ValidationIssue
...
class TestRuleTruthClassification:
    ...
    def test_fm005_type_mismatch_is_error(self, tmp_path: Path) -> None:
        ...
        """
        skill_md = tmp_path / "SKILL.md"
        # `description` is typed `str` on SkillFrontmatter; a mapping value makes
        # Pydantic raise string_type, which _pydantic_error_to_validation_issue
        # maps to FM005.
        skill_md.write_text(
            dedent("""\
            ---
        ...
        fm005 = _find_issue_by_code(result, FM005)
        assert fm005 is not None, f"Expected FM005 for a type mismatch, got: {result.errors}"
        assert fm005.severity == "error", f"FM005 should be error (schema violation), got {fm005.severity}"
